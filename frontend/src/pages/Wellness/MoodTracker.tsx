...
const MoodTracker: React.FC = () => {
  ...
  const loadMoodHistory = async () => {
    try {
      setLoading(true);
      console.log('Loading mood history from backend...');
      
      const entries = await moodService.getMoodHistory(5);
      console.log('Mood history loaded:', entries);
      
      // Convert backend format to frontend format
      const formattedEntries: MoodData[] = entries.map(entry => ({
        id: entry.id,
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        console.log('Video element after setup:', video);
  ...
  const analyzeMood = async (imageData: string) => {
    setAnalyzing(true);
    try {
      console.log('Analyzing mood using backend API...');
      
      // Use the backend emotion analysis service
      const analysisResult = await moodService.analyzeEmotion(imageData);
      ...
      toast({
        ...
        description: `Detected emotion: ${analysisResult.emotion} (${Math.round(analysisResult.confidence * 100)}% confidence)`,
      });
    } catch (error) {
  ...
  const saveMoodEntry = async () => {
    ...
    try {
      console.log('Saving mood entry to backend...');
      
      // Save to backend using the mood service
      const savedEntry = await moodService.saveMoodEntry({
        emotion: currentAnalysis.emotion,
        confidence: currentAnalysis.confidence,
        emotions_breakdown: currentAnalysis.emotions,
      ...
      const newEntry: MoodData = {
        ...
        imageData: savedEntry.image_data,
      ...
      toast({
        title: 'Save Error',
        description: 'Failed to save mood entry to database',
        variant: 'destructive',
      });
    }
  };
            ...
            <CardContent className="space-y-4">
                ...
                {isRecording ? (
                  <>
                    ...
                    <div className="absolute bottom-2 left-2 right-2 bg-black bg-opacity-50 text-white text-center py-2 rounded">
                      <p className="text-sm">Position your face in the center and click "Capture Mood"</p>
                    </div>
                  </>
                ) : capturedImage ? (
                  <img
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-400 mt-2">Make sure to allow camera permissions</p>
                    </div>
                  </div>
                )}
              ...
              {!isRecording && (
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Camera not working?</strong> Make sure to:
            ...
            {moodData.length === 0 ? (
              ...
              <div className="space-y-4">
                {moodData.slice(0, 5).map((entry, index) => (
                  <div key={entry.id || `mood-entry-${index}`} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg">
